...
//                    list.clear();
    ...
    private void initDataFromDB(List<ApplicationInfo> list) {
        ...
        List<ResolveInfo> listapp = loadApps();
        for (ResolveInfo resolveInfos : listapp) {
            ApplicationInfo info = new ApplicationInfo();
            info.setId(0 + "");
            info.setTitle(resolveInfos.activityInfo.loadLabel(getPackageManager()).toString());
            info.setOrder(0);
            info.setDrawable(resolveInfos.activityInfo.loadIcon(getPackageManager()));
            info.setPackageName(resolveInfos.activityInfo.packageName);
