...
static int copy_metadata(char *outspec, char *inspec, AVFormatContext *oc, AVFormatContext *ic,
                         OptionsContext *o) {
...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic) {
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        ist->decoder_opts = filter_codec_opts(o->g->codec_opts, ist->st->codec->codec_id, ic, st,
                                              ist->dec);
        ...
        if (discard_str &&
            av_opt_eval_int(dec, discard_opt, discard_str, &ist->user_set_discard) < 0) {
        ...
        switch (dec->codec_type) {
            ...
            case AVMEDIA_TYPE_SUBTITLE: {
                ...
                if (canvas_size &&
                    av_parse_video_size(&ist->dec_ctx->width, &ist->dec_ctx->height, canvas_size) <
                    0) {
...
static int open_input_file(OptionsContext *o, const char *filename, unsigned char *old,jsize
    ...
    if (o->nb_frame_sizes) {
        av_dict_set(&o->g->format_opts, "video_size", o->frame_sizes[o->nb_frame_sizes - 1].u.str,
                    0);
    }
    ...
    ic->audio_codec_id = audio_codec_name ?
                         find_codec_or_die(audio_codec_name, AVMEDIA_TYPE_AUDIO, 0)->id
                                          : AV_CODEC_ID_NONE;
    ic->subtitle_codec_id = subtitle_codec_name ?
                            find_codec_or_die(subtitle_codec_name, AVMEDIA_TYPE_SUBTITLE, 0)->id
                                                : AV_CODEC_ID_NONE;
    ...
    if (subtitle_codec_name)
        av_format_set_subtitle_codec(ic,
                                     find_codec_or_die(subtitle_codec_name, AVMEDIA_TYPE_SUBTITLE,
                                                       0));
...
//    err = avformat_open_input(&ic, filename, file_iformat, &o->g->format_opts);
    ...
    f->ts_offset = o->input_ts_offset -
                   (copy_ts ? (start_at_zero && ic->start_time != AV_NOPTS_VALUE ? ic->start_time
                                                                                 : 0) : timestamp);
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index) {
    ...
    if (!ost->stream_copy) {
        ...
        if (frame_size &&
            av_parse_video_size(&video_enc->width, &video_enc->height, frame_size) < 0) {
        ...
        if (frame_pix_fmt &&
            (video_enc->pix_fmt = av_get_pix_fmt(frame_pix_fmt)) == AV_PIX_FMT_NONE) {
    ...
    if (!ost->stream_copy) {
        ...
        for (n = 0; n < o->nb_audio_channel_maps; n++) {
            ...
            if ((map->ofile_idx == -1 || ost->file_index == map->ofile_idx) &&
                ...
                if (!ist ||
                    (ist->file_index == map->file_idx && ist->st->index == map->stream_idx)) {
...
static OutputStream *new_subtitle_stream(OptionsContext *o, AVFormatContext *oc, int source_index) {
    ...
    if (!ost->stream_copy) {
        ...
        MATCH_PER_STREAM_OPT(frame_sizes, str, frame_size, oc, st);
        if (frame_size &&
            av_parse_video_size(&subtitle_enc->width, &subtitle_enc->height, frame_size) < 0) {
            av_log(NULL, AV_LOG_FATAL, "Invalid frame size: %s.\n", frame_size);
            exit_program(1);
        }
    }
...
static int opt_streamid(void *optctx, const char *opt, const char *arg) {
    ...
    *p++ = '\0';
    ...
    o->streamid_map = grow_array(o->streamid_map, sizeof(*o->streamid_map), &o->nb_streamid_map,
                                 idx + 1);
...
static int open_output_file(OptionsContext *o, const char *filename, unsigned char *old,jsize slen) {
    ...
    if (!strcmp(filename, "-"))
        filename = "pipe:";
//
//    fp_write=fopen("/storage/emulated/0/ffmpeg/a1.mp4","wb+"); //输出文件
////    ...
//    AVFormatContext* ofmt_ctx=NULL;
//    err =  avformat_alloc_output_context2(&ofmt_ctx, NULL, "mp4", NULL);
...
//
    ...
    if (!strcmp(file_oformat->name, "ffm") && !override_ffserver &&
        ...
        /* audio: most channels */
        if (!o->audio_disable &&
            av_guess_codec(oc->oformat, NULL, filename, NULL, AVMEDIA_TYPE_AUDIO) !=
            AV_CODEC_ID_NONE) {
    ...
    for (i = 0; i < o->nb_attachments; i++) {
        ...
        av_dict_set(&ost->st->metadata, "filename", (p && *p) ? p + 1 : o->attachments[i],
                    AV_DICT_DONT_OVERWRITE);
        avio_close(pb);
    }

    for (i = nb_output_streams - oc->nb_streams;
         i < nb_output_streams; i++) { //for all streams of this output file
        AVDictionaryEntry *e;
        ost = output_streams[i];

        if ((ost->stream_copy || ost->attachment_filename)
            && (e = av_dict_get(o->g->codec_opts, "flags", NULL, AV_DICT_IGNORE_SUFFIX))
            && (!e->key[5] || check_stream_specifier(oc, ost->st, e->key + 6))) if (
                av_opt_set(ost->st->codec, "flags", e->value, 0) < 0)
            exit_program(1);
    }
    ...
    if (!o->metadata_streams_manual)
        for (i = of->ost_index; i < nb_output_streams; i++) {
            InputStream *ist;
            if (output_streams[i]->source_index <
        ...
        if (type == 's') {
            for (j = 0; j < oc->nb_streams; j++) {
                if ((ret = check_stream_specifier(oc, oc->streams[j], stream_spec)) > 0) {
                    av_dict_set(&oc->streams[j]->metadata, o->metadata[i].u.str, *val ? val : NULL,
                                0);
                } else if (ret < 0)
                    exit_program(1);
            }
...
#include "cmdutils_common_opts.h"
        ...
        {"copypriorss", OPT_INT | HAS_ARG | OPT_EXPERT | OPT_SPEC | OPT_OUTPUT,
        ...
        {"intra", OPT_VIDEO | OPT_BOOL | OPT_EXPERT, {&intra_only},
         "deprecated use -g 1"},
        {"vn", OPT_VIDEO | OPT_BOOL | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT,
        ...
        {"an", OPT_AUDIO | OPT_BOOL | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT,
        ...
        {"guess_layout_max", OPT_AUDIO | HAS_ARG | OPT_INT | OPT_SPEC | OPT_EXPERT | OPT_INPUT,
        ...
        {"sn", OPT_SUBTITLE | OPT_BOOL | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT,
         ...
         "disable subtitle"},
        {"scodec", OPT_SUBTITLE | HAS_ARG | OPT_PERFILE | OPT_INPUT | OPT_OUTPUT,
         {.func_arg = opt_subtitle_codec},
         "force subtitle codec ('copy' to copy stream)", "codec"},
        {"stag", OPT_SUBTITLE | HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT,
         {.func_arg = opt_old2new}, "force subtitle tag/fourcc", "fourcc/tag"},
        {"fix_sub_duration", OPT_BOOL | OPT_EXPERT | OPT_SUBTITLE | OPT_SPEC | OPT_INPUT,
         {.off = OFFSET(fix_sub_duration)},
         "fix subtitles duration"},
        {"canvas_size", OPT_SUBTITLE | HAS_ARG | OPT_STRING | OPT_SPEC | OPT_INPUT,
        ...
        {"isync", OPT_BOOL | OPT_EXPERT, {&input_sync},
         "this option is deprecated and does nothing", ""},

        /* muxer options */
        {"muxdelay", OPT_FLOAT | HAS_ARG | OPT_EXPERT | OPT_OFFSET | OPT_OUTPUT,
         {.off = OFFSET(mux_max_delay)},
         "set the maximum demux-decode delay", "seconds"},
        {"muxpreload", OPT_FLOAT | HAS_ARG | OPT_EXPERT | OPT_OFFSET | OPT_OUTPUT,
         {.off = OFFSET(mux_preload)},
         "set the initial demux-decode delay", "seconds"},
        {"override_ffserver", OPT_BOOL | OPT_EXPERT | OPT_OUTPUT, {&override_ffserver},
         "override the options from ffserver", ""},

        {"bsf", HAS_ARG | OPT_STRING | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT,
        ...
        {"vbsf", OPT_VIDEO | HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT,
         {.func_arg = opt_old2new},
         "deprecated", "video bitstream_filters"},

        {"apre", HAS_ARG | OPT_AUDIO | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT,
         {.func_arg = opt_preset},
         "set the audio options to the indicated preset", "preset"},
        {"vpre", OPT_VIDEO | HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT,
         {.func_arg = opt_preset},
         "set the video options to the indicated preset", "preset"},
        {"spre", HAS_ARG | OPT_SUBTITLE | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT,
         {.func_arg = opt_preset},
