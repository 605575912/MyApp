...
public class DateView extends View {
	...
	public void init(Context context) {
		...
		paint_LTGRAY.setStrokeWidth(0.1f);
	...
	void canfirt(int i, Canvas canvas) {
		...
		sCanvas.drawText(list.get(e), getWidth() - getPaddingRight(), y0
				+ baseline, paint_LTGRAY);
		y0 = (float) (y0 - rectheight);
		float scroe = y0 / rectheight / 2;
		matrix.postScale(1f, scroe + 0.6f, getWidth() - getPaddingRight(), y0
				+ +baseline);
		matrix.postSkew(0f, 0.2f, getWidth() - getPaddingRight(),
				(y0 + +baseline));
		if (y0 != rectheight / 2) {
			sCanvas.setMatrix(matrix);
		}
		e = (index) % list.size();
		sCanvas.drawText(list.get(e), getWidth() - getPaddingRight(),
				(int) (y0 + baseline), paint_LTGRAY);
	...
	void canmid(int i, Canvas canvas) {
		...
		sCanvas.drawText(list.get(e), getWidth() - getPaddingRight(),
				(int) (y0 + baseline), paint_black);
		y0 = (float) (rectheight + y0);
		e = (index + 2) % list.size();

		sCanvas.drawText(list.get(e), getWidth() - getPaddingRight(), y0
				+ baseline, paint_black);
		sCanvas.drawText(hour, getWidth() - 2 * getPaddingRight(), rectheight
				/ 2 + textbaseline, paint_gray_text);
		canvas.drawBitmap(canvasbitmap,
				getWidth() / 2 - canvasbitmap.getWidth() / 2, (int) rectheight
						* i + 1, null);
	}
	...
	void canend(int i, Canvas canvas) {
		...
		matrix.setScale(1f, scroe + 0.6f, getWidth() - getPaddingRight(), y0
				+ +baseline / 2);
		matrix.postSkew(0.1f, 0f, getWidth() - getPaddingRight(),
				(y0 + +baseline));
		if (y0 != rectheight / 2) {
			sCanvas.setMatrix(matrix);
		}
